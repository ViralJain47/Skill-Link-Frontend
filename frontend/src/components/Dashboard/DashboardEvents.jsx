...
import { FaCalendarAlt, FaClock, FaMapMarkerAlt, FaArrowRight, FaPlus, FaEdit, FaTrash } from 'react-icons/fa';
...
function DashboardEvents() {
  ...
  return (
    <section id="events" className="h-screen bg-gray-50 py-20">
      <motion.div
        ref={eventsRef}
      ...
      >
        {/* Header Section */}
        <motion.div className="flex flex-col items-center mb-16" variants={fadeInUp}>
          ...
          <p className="text-gray-600 mt-4 max-w-2xl text-center">
            Join us for these exciting opportunities to learn, connect, and grow with fellow students and industry experts.
          </p>
        ...
        {/* Error Message */}
        {error && (
          <motion.div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-6" variants={fadeInUp}>
            <p>{error}</p>
          </motion.div>
        )}
        ...
        {loading ? (
          ...
          <>
            ...
            {events && events.length > 0 ? (
                ...
                {events.map((event) => (
                  ...
                  >
                    <div className="relative h-48 overflow-hidden">
                      ...
                      />
                      <div className={`absolute top-4 left-4 bg-${getCategoryColor(event.type)}-600 text-white rounded-lg px-3 py-1 text-sm font-medium`}>
                        {event.type || 'Event'}
                      </div>
                      <div className={`absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-${getCategoryColor(event.type)}-700 rounded-lg px-3 py-1 text-sm font-bold`}>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center text-gray-500 mb-6">
                        ...
                        <span className="text-sm">{new Date(event.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} ({event.duration} mins)</span>
                      ...
                      <div className="flex justify-between items-center">
                            ...
                            <>
                              {event.participants.slice(0, 2).map((participant, index) => (
                                <div key={index} className="w-8 h-8 rounded-full bg-gray-200 border-2 border-white overflow-hidden">
                                  <img 
                                    src={participant.avatar || '/api/placeholder/50/50'} 
                        ...
                        <button 
                          className={`bg-${getCategoryColor(event.type)}-50 hover:bg-${getCategoryColor(event.type)}-100 text-${getCategoryColor(event.type)}-700 font-medium py-2 px-4 rounded-lg transition-colors duration-300`}
                        >
                          {event.registrationFee > 0 ? `Register ($${event.registrationFee})` : 'RSVP'}
                        </button>
                      </div>
                    </div>
                  </motion.div>
              ...
              <motion.div className="text-center py-12 text-gray-500" variants={fadeInUp}>
        ...
        <motion.div className="mt-12 text-center" variants={fadeInUp}>
          <a href="#all-events" className="inline-flex items-center text-emerald-600 font-medium hover:text-emerald-700 transition-colors">
            View all events
