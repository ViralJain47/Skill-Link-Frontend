...
import { Button } from "../"; // Assuming Button correctly handles className and onClick
...
function ContactUs() {
  ...
  return (
    ...
    >
      {/* Left Side */}
      <div className="flex-col justify-center items-start w-full md:w-auto">
        {" "}
        {/* Control width on mobile */}
        <div className="flex flex-col justify-center item-start mb-10 md:mb-15">
          {" "}
          {/* Consistent margin */}
          <h2 className="text-3xl sm:text-4xl font-semibold">
            Send us a message
          </h2>
          <p className="mt-3 sm:mt-4 text-neutral-700">
            {" "}
            {/* Slightly adjusted margin/color */}
        ...
        >
          <img
            src={Email}
            alt="Email icon"
            className="w-7 sm:w-8 transition-transform duration-300 group-hover:rotate-[15deg]"
          />{" "}
          {/* Subtle icon animation on hover */}
          <p className="text-neutral-800 text-base sm:text-xl">
            {" "}
            {/* Responsive text size */}
            Mail us at:{" "}
            <span className="text-black font-medium">
              support.skilllink@gmail.com
            </span>{" "}
            {/* Added font-medium */}
          </p>
        </a>
      ...
      <div className="w-full md:w-1/2">
        ...
        <form
          className="flex flex-col w-full justify-center items-center gap-8 md:gap-10"
        ...
        >
          ...
          <div className="w-full flex flex-col gap-2">
            ...
            />
            <p className="text-sm text-neutral-600">Max 500 Characters</p>{" "}
            {/* Adjusted color/size */}
          </div>

          {/* Buttons */}
          <div className="w-full mt-5 flex gap-4 sm:gap-7 justify-start items-center">
            {" "}
            {/* Changed justify-baseline to start */}
